 ...
 */
//BEGIN <lcm> <DATE20130904> <add lcm info> zhangxiaofei
static ssize_t lcm_info_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
    char  vl_lcm_array[256] ={ 0};
    char  *p_lcm_info = NULL;

    //if(DISP_IsLcmFound())		//add by lishengli 20140426   get lcd info
    //{
        p_lcm_info = DISP_GetLCMId();
    //}
    
    if(p_lcm_info)
    {   
        sprintf(vl_lcm_array, "%s",p_lcm_info);	
    }
    else
    {   
        sprintf(vl_lcm_array, "%s","unknow lcd");	
    }

    return sprintf(buf, "%s", vl_lcm_array);

}
static DEVICE_ATTR(lcm_info, 0444, lcm_info_show, NULL);
//END <lcm> <DATE20130904> <add lcm info> zhangxiaofei
...
static int mtkfb_probe(struct device *dev)
{
    ...
    {
        printk("Register dev_attr_lcm_info failed");
    }
    else
    {
        printk("Register dev_attr_lcm_info success");
    }
    //END <lcm> <DATE20130904> <add lcm info> zhangxiaofei
    return 0;

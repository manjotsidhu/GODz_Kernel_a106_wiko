...
#define GPIO_SPEAKER_EN2_PIN GPIO45
...
bool Speaker_Init(void)
{
   ...
   mt_set_gpio_mode(GPIO_SPEAKER_EN2_PIN,GPIO_MODE_00);  // gpio mode
   mt_set_gpio_pull_enable(GPIO_SPEAKER_EN2_PIN,GPIO_PULL_ENABLE);
   PRINTK("-Speaker_Init Success");
   return true;
}
...
void Sound_Speaker_Turnon(int channel)
{
    ...
    //LINE <waa2145> <DATE20130717> <waa2145 Non-chip mode2> zhangxiaofei
    mt_set_gpio_dir(GPIO_SPEAKER_EN2_PIN,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_SPEAKER_EN2_PIN,GPIO_OUT_ZERO); // high

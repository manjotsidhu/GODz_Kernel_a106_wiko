...
typedef struct
{
    u16 pid;                 //product id   //
    u16 vid;                 //version id   //
    u16 sid;			//sensor id	//add by lishengli 20140103
} st_tpd_info;
#pragma pack()
...
//LINE<JIRA_ID><DATE20140327><BUG_INFO>zenghaihui
static u8 gtp_clk_file_buf[6] = {0x00};
static u8 gtp_ref_file_buf[512 + 256] = {0x00};
...
s32 gtp_read_version(struct i2c_client *client, u16 *version)
{
    ...
    GTP_INFO("gtp_read_version lishengli555  : tpd_info.vid =  %d, tpd_info.pid = %d",
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    tpd_info.sid = sensor_id;			//add by lishengli 20140103
...
#endif
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            if (opr_buf[0] < 200)				//add by lishengli 20140109
            {
                grp_cfg_version = send_cfg_buf[sensor_id][0];       // backup group config version
                //send_cfg_buf[sensor_id][0] = 0x00;				//add by lishengli 20140109
                fixed_config = 0;
            }
...
static u8 gtp_bak_ref_proc(struct i2c_client *client, u8 mode)
{
    ...
    if((RECOVERY_BOOT == get_boot_mode()) || (gtp_ref_retries >= GTP_CHK_FS_MNT_MAX))
    {
...
//LINE<JIRA_ID><DATE20140327><BUG_INFO>zenghaihui
    ...
    if(GTP_BAK_REF_SEND == mode)
    {
        //flp->f_op->llseek(flp, 0, SEEK_SET);
        //ret = flp->f_op->read(flp, (char *)refp, ref_len, &flp->f_pos);
        memcpy((char *)refp, gtp_ref_file_buf, ref_len);
    }
#endif
    ...
    if(GTP_BAK_REF_STORE == mode)
    {
        ...
        flp->f_op->write(flp, (char *)refp, ref_len, &flp->f_pos);
        #else
        memcpy(gtp_ref_file_buf, (char *)refp, ref_len);
        #endif
    }
    else
    {
...
static u8 gtp_main_clk_proc(struct i2c_client *client)
{
    ...
    if ( RECOVERY_BOOT != get_boot_mode() )
    {
        ...
        {
            //GTP_DEBUG("[gtp_main_clk_proc]/data mounted !!!!");
            //flp = filp_open(GTP_MAIN_CLK_PATH, O_RDWR | O_CREAT, 0777);
            //if (!IS_ERR(flp))
            {
                //flp->f_op->llseek(flp, 0, SEEK_SET);
                //ret = flp->f_op->read(flp, (char *)gtp_clk_buf, 6, &flp->f_pos);
                //if(ret > 0)
                {
                    memcpy(gtp_clk_buf, gtp_clk_file_buf, 6);
                    ret = gtp_check_clk_legality();
                    if(SUCCESS == ret)
                    {
                        GTP_DEBUG("[gtp_main_clk_proc]Open & read & check clk file success.");
    ...
    {
        GTP_INFO("recovery mode, send default main clock: 72.");
        clk_cal_result = 72;    // default main clock 
    }
...
send_main_clk:
    ...
    //if (flp && !IS_ERR(flp))
    {
        ...
        //close_ret = filp_close(flp, NULL);
        memcpy(gtp_clk_file_buf, gtp_clk_buf, 6);
...
//LINE<JIRA_ID><DATE20140327><gt950 dl fw in tpd_power_on>zenghaihui
#if 0 // GTP_AUTO_UPDATE
...
{
#if GTP_COMPATIBLE_MODE
    if ((CHIP_TYPE_GT9F == gtp_chip_type) && (RECOVERY_BOOT != get_boot_mode()))
    {
...
GTP_INFO("Gt9xx_tpd_get_fw_vendor_name lishengli555  : tpd_info.sid =  %d",
    ...
    switch(tpd_info.sid)
    {
        ...
        //LINE<JIRA_ID><DATE20140504><add jtouch tp>zenghaihui
        case 5:

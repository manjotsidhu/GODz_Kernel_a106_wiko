...
//Add these Define
#define IAP_PORTION         1           //upgrade  FW
#define VIA_ROM             0
...
#endif
static struct tpd_driver_t tpd_device_driver;//LINE<tp elan><tp version><20130904>yinhuiyong
...
static uint8_t file_fw_data_dijing[] = {
0//#include "fw_data_dijing.i"
};
static uint8_t file_fw_data_yeji[] = {
0//#include "fw_data_yeji.i"
};
static uint8_t file_fw_data_huaruichuan[] = {
#include "fw_data_huaruichuan.i"
};
static uint8_t* file_fw_data = NULL;
//FW ID CHECK ----start by baojun.fu
#endif

#define FW_ID_HUARUICHUAN		0x4700 //0x603B
#define FW_ID_DIJING			0x2747
#define FW_ID_YEJI				0x6F3B
...
//{ iap-ram
		...
		case IOCTL_IAPRAM_CHECKSUM:
			printk("[elan] IOCTL_IAPRAM_CHECKSUM,%d\n",(int*)arg);
			//len = 1;
			if (copy_from_user(buf, (const void*)arg, 4))
			{
				printk("[elan] copy_from_user error\n");
				return -EFAULT;
			}
			printk("[elan] IOCTL_SET_COMMAND:%x:%x:%x:%x\n",
...
*/
         ...
         {
			RECOVERY=0x80;
			//BEGIN<elan tp><add yeji vendor><date20131108>yinhuiyong
			TP_ID = FW_ID =  buf_recv[5] << 8 | buf_recv[4];// RECOVERY MODE for yeji TP 2013/11/27 no need to read FW ID
			printk("[elan] FW_ID = %x\r\n", FW_ID);
			//END<elan tp><add yeji vendor><date20131108>yinhuiyong
			rc = i2c_master_recv(client, buf_recv, 8);
...
static int write_check_fwid_to_rom(struct i2c_client *client, uint8_t *cmd, size_t size)
{
    ...
    rc = elan_ktf2k_ts_get_data(client, get_cmd, buf_recv, 4);
    if (rc < 0){
       return rc;
    }
    major = buf_recv[2];
    minor = buf_recv[3];
	//FW_ID_HUARUICHUAN		0x603B
	//FW_ID_DIJING			0x2747
	//FW_ID_YEJI			0x6F3B

	printk("[elan] major = %d, minor = %d      ************************\n",major, minor);


    if( major != 0X47 )//not fit for  FW ID then write it
    {
...
static int __fw_packet_handler(struct i2c_client *client)
{
	...
	uint8_t cmd_check_fwid[] = { 0x54, 0XD2, 0xFF,0xFF };	/* Get Check FWID *///LINE<elan tp><add yeji vendor><date20131108>yinhuiyong
...
// Firmware ID
		 ...
		 printk("[elan] cmd_check_fwid[2] = %d, cmd_check_fwid[3] = %d    ****************\n",major, minor);
         rc = write_check_fwid_to_rom(client, cmd_check_fwid , 4);  // write check fwid info
         FW_ID = major << 8 | minor;/*Get Check FWID */
...
// Bootcode version
	 ...
	 //Begin<tp elan><tp version><20130904>yinhuiyong
		{
		extern char tpd_desc[50];
		extern int tpd_fw_version;
		if(FW_ID == FW_ID_HUARUICHUAN)
		{
			sprintf(tpd_desc, "%s","HUARUICHUAN");
		}else if(FW_ID == FW_ID_YEJI)
		{
			sprintf(tpd_desc, "%s","YEJI");
		}else if(FW_ID == FW_ID_DIJING)
		{
			sprintf(tpd_desc, "%s","DIJING");
		}
		printk("[elan] tpd_desc_show = %d\n",FW_VERSION);
		tpd_fw_version = FW_VERSION;
		}
	//End<tp elan><tp version><20130904>yinhuiyong
  ...
  if(!pReadData){
	printk("mtk-tpd:[elan] dma_alloc_coherent failed!\n");
        ...
        MTK_TP_DEBUG("[elan_debug] %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7],buf[8], buf[9], buf[10], buf[11], buf[12], buf[13], buf[14], buf[15],buf[16], buf[17]);
         ...
         //mdelay(1);--mask for performence--by baojun.fu 20131025
...
IAP_RESTART:
    ...
    curIndex = 0;
	//reset
	printk("[ELAN] IAPRAM_MODE\n");
	data= IAP_RAM_ADDR;
//	dev_dbg(&client->dev, "[ELAN] %s: address data=0x%x \r\n", __func__, data);
	res = EnterISPMode( /*private_ts->*/client, iapRam_cmd );	 //enter ISP mode
	if( res != 0 )
	{
		printk("[elan] EnterISPMode error code = %d\n",res);
		return E_FD;
	}	
//mdelay(500);

	// Send Dummy Byte	
	printk("[ELAN] send one byte data:%x,%x",client->addr,data);
	res = i2c_master_send(client, &data,  sizeof(data));
	if( res!=sizeof(data) )
	{
		printk("[ELAN] dummy error code = %d\n",res);
...
PAGE_REWRITE:
	...
	//			printk("curIndex =%d\n",curIndex);
				...
				if( res != 0 )
				{
					printk("[elan] IAP-RAM WRITE error code = %d\n",res);
					return E_FD;
				}
			}
			else
...
#endif
		printk("[ELAN] IAP_RAM successfully!\n");
	return 0;
}
...
/*	rc = elan_ktf2k_ts_poll(client);
	...
	if(buf_recv[0]== 0xcc && buf_recv[1]== 0xcc && buf_recv[2]==0xcc && buf_recv[3]==0xcc)
	{
		...
		{
			printk("[elan] checksum is right\n");
			return 1;
		}
		else
		{
			printk("[elan] checksum is Error, Need to be updated\n");	
			return 0;
		}
	}
/*
...
//-----ps on add by baojun.fu---e
	...
	else if (buf[0] == 0xcc) 
	{
		...
		printk("[elan] %s: via rom data %2x:%2x:%2x:%2x\n", __func__, file_iapRam_data[2241], file_iapRam_data[2240], file_iapRam_data[2239], file_iapRam_data[2238]);
		...
		if(buf[0]== 0xcc && buf[1]== 0xcc && buf[2]==0xcc && buf[3]==0xcc)
		{
			...
			else	
			{
				...
				printk("[elan] %s Enter into normal report system\n", __func__);
				elan_ktf2k_ts_iapRam_continue(i2c_client);
				//msleep(500);
			}
...
/* FW ID & FW VER*/
    ...
    printk(" FW_VERSION=0x%x,   New_FW_VER=0x%x \n",  FW_VERSION  , New_FW_VER); 
#endif     
    if(FW_ID == New_FW_ID)//RECOGNISE TP 
	{
		//* for firmware auto-upgrade*/        
        if (New_FW_VER > (FW_VERSION)) {
				printk("FW_Version is different!");
				Update_FW_One(private_ts->client, RECOVERY);
		} else if(FW_VERSION == 0) {                       
				printk("FW_Version is not read ok!");
				RECOVERY=0x80;                  
				Update_FW_One(private_ts->client, RECOVERY);           
		}
	}
...
//      int button_state = 0;
         ...
         int index = 0;
         int i =0;
         MTK_TP_DEBUG("mtk-tpd[elan] interrupt touch_event_handler\n");
...
//         #endif
       //  client->addr |= I2C_ENEXT_FLAG;
         ...
         printk("mtk-tpd:[elan] %s:client , TPD_DEVICE = ektf2k\n",__func__);
...
*/
   // printk("ELAN enter tpd_probe_ ,the i2c addr=0x%x", client->addr);
    mt_set_gpio_mode(GPIO_CTP_RST_PIN, 0);
    mt_set_gpio_dir(GPIO_CTP_RST_PIN, 1);
    mt_set_gpio_out(GPIO_CTP_RST_PIN, 0);
	msleep(10);
...
// Setup Interrupt Pin
         ...
         //mt_eint_set_hw_debounce(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);
	 	 //mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, tpd_eint_interrupt_handler, 0);         
...
//#endif
         ...
         {
            //FW ID CHECK ----start by baojun.fu
           // printk(KERN_ERR "[elzan] %s: TP , FW_ID: 0x%4.4x \n", __func__, FW_ID);
             if(FW_ID == FW_ID_HUARUICHUAN) {
                     printk(KERN_ERR "[elan] %s: TP is FW_ID_HUARUICHUAN, FW_ID: 0x%4.4x \n", __func__, FW_ID);
                     file_fw_data = file_fw_data_huaruichuan;
             }else if (FW_ID == FW_ID_YEJI) {
                     printk(KERN_ERR "[elan] %s: TP is FW_ID_DIJING, FW_ID: 0x%4.4x \n", __func__, FW_ID);
                     file_fw_data = file_fw_data_yeji;
             }else {
                     printk(KERN_ERR "[elan] TP ID is error: no support!\n");
                     return -EIO;//TP ID is error
             }
            //FW ID CHECK ----end   by baojun.fu
             mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
...
static int tpd_suspend(struct i2c_client *client, pm_message_t message)
{
	...
	if (work_lock == 1)//if update FW stop suspend 2014/1/3
	{
		return -1;
	}
...
*/
//BEGIN <touch panel> <DATE20130929> <touch panel version info> yinhuiyong
extern char tpd_desc[50];
int ektf2k_tpd_get_fw_version( void )
{
    return FW_VERSION;
}
...
//END <touch panel> <DATE20130929> <touch panel version info> yinhuiyong
...
{
         printk("mtk-tpd ekft2k touch panel driver init\n");
...
static int __init tpd_driver_init(void)
{
    ...
    printk("MediaTek ekft2k touch panel driver init\n");

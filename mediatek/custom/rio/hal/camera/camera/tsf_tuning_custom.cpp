...
isEnableTSF(MINT32 const i4SensorDev)
{
 ...
 // 0: LSC
 // 1: TSF
 // 2: TSF, flash switch to LSC return
 //return 2; // LINE <><2014.1.27><Use TSF excpet flash light> Jiangde
 //  return 0; // LINE <><2014.2.14><Close TSF to avoid mosaic when open camera> Jiangde
    ...
    if(strstr(module_name,"ov5648mipi_sunwin"))
    {
        XLOGD("isEnableTSF-1, module_name == %s\n",module_name);
        return 2; // LINE <><2014.1.27><Use TSF excpet flash light> Jiangde
    }
	else
    {
       XLOGD("isEnableTSF-2, module_name == %s\n",module_name);
       return 0; 
    }

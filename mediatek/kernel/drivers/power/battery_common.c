	...
	else if(g_batt_temp_status == TEMP_POS_LOW)
	{
		if (BMT_status.temperature >= MIN_CHARGE_TEMPERATURE_PLUS_X_DEGREE) {
			...
			BMT_status.bat_charging_state=CHR_PRE;
                ...
                //LINE<JIRA_ID><DATE20140404><reset warnning status>zenghaihui
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)
#else
//BEGIN <charge temperature> <DATE20130621> <charging temperature> zhangxiaofei

    //if(BMT_status.temperature >= 20) // only for test
	if(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)
#endif	//#if defined(MTK_JEITA_STANDARD_SUPPORT)
    {
        g_BatteryNotifyCode |= 0x0002;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) over of range\n", BMT_status.temperature);
    }
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
#if defined(MTK_JEITA_STANDARD_SUPPORT)
	else if(BMT_status.temperature < TEMP_NEG_10_THRESHOLD && BMT_status.charger_exist== KAL_TRUE)//modified by mickal.ma
#else
	else if(BMT_status.temperature <= MIN_CHARGE_TEMPERATURE && BMT_status.charger_exist== KAL_TRUE)
#endif
    {
		g_BatteryNotifyCode |= 0x0020;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) under of range\n", BMT_status.temperature);
	}
#endif
//END <charge temperature> <DATE20130621> <charging temperature> zhangxiaofei	
    else
    {
        g_BatteryNotifyCode &= ~(0x0002);
    }
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
    ...
    do
    {
        ...
        wait_event_interruptible(charger_hv_detect_waiter, (charger_hv_detect_flag == KAL_TRUE));
       	...
       	if ((upmu_is_chr_det() == KAL_TRUE))
        {
            //check_battery_exist();	//Line modified by mickal.ma
        }

...
//BEGIN <ftm> <DATE2013118> <ftm ker V+ + V-> mickal.ma
void boot_mode_menu_select2()
{
          int select = 0;  // 0=recovery mode, 1=fastboot.  2=normal boot
          const char* title_msg = "Select Boot Mode:\n[VOLUME_UP to Recovery Mode.  VOLUME_DOWN to factory Mode.]\n\n";
          video_clean_screen();
	   ...
	   while(mtk_detect_key(MT65XX_MENU_SELECT_KEY) || mtk_detect_key(MT65XX_MENU_OK_KEY))
	   {
          ...
          while(1)
          {
		if(mtk_detect_key(MT65XX_MENU_SELECT_KEY)&&!mtk_detect_key(MT65XX_MENU_OK_KEY))//VOL_UP
		{
			select = 0;
			break;
		}
		else if(mtk_detect_key(MT65XX_MENU_OK_KEY)&&!mtk_detect_key(MT65XX_MENU_SELECT_KEY))//VOL_DOWN,
		{
			select = 1;
			break;
		}
		...
		mtk_wdt_restart();
          ...
          video_set_cursor(2 +8, 0);
          return;
//END <ftm> <DATE2013118> <ftm ker V+ + V-> mickal.ma
}

...
#warning "[TODO] perform a dummy compressed-image callback or CAMERA_MSG_ERROR to inform app of end of capture?"
    ...
    else
    {
        ...
        shotParam.mi4Flip               = pParamsMgr->getInt(CameraParameters::KEY_FLIP);	//add jpg flip
        shotParam.mi4SmartSkin               = pParamsMgr->getInt(CameraParameters::KEY_SMARTSKIN);	//add access
        shotParam.mi4WhiteNing               = pParamsMgr->getInt(CameraParameters::KEY_WHITENING);	//add access
        shotParam.mi4EnableAccess               = pParamsMgr->getInt(CameraParameters::KEY_ENABLEACCESS);	//add access
        #endif
        //

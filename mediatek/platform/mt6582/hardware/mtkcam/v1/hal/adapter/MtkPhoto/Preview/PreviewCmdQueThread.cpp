...
static int morpho_init_w=0,  morpho_init_h=0;
...
start()
{
    ...
    #if ACCESS_BEAUTIFIER
    if(mSensorInfo.isSub() == true)//sub camera
    {
...
#if ACCESS_BEAUTIFIER
if (mSensorInfo.isSub() == true)
    {
    	if(dispNode.getImgBuf() != 0)
	{
		width = dispNode.getImgBuf()->getImgWidth();
		height = dispNode.getImgBuf()->getImgHeight();
		MY_LOGE("dispNode.getImgBuf()->getImgHeight() == %d\n",height);
	  MY_LOGE("dispNode.getImgBuf()->getImgWidth() == %d\n",width);
	  ...
	  smartskin  = mspParamsMgr->getInt(CameraParameters::KEY_SMARTSKIN);	//add access
	   ...
	   if ((morpho_init_w !=width)||(morpho_init_h !=height) )
	        {
		MY_LOGE("before pr_access_beautifier_initialize %d * %d\n", width, height);
		morpho_init_w = width;
		morpho_init_h = height;
